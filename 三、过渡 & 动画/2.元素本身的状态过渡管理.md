Vue 的过渡系统提供了非常多简单的方法设置进入、离开和列表的动效。那么对于数据**元素本身的动效**呢，比如：
...
> 我们使用Vue的响应系统来同步更新数据显示，使用侦听器来监听数据变化，判断是否执行动画效果，而具体的过渡效果则交给第三方库来实现。

## 动态状态过渡

就像 Vue 的过渡组件一样，数据背后状态过渡会实时更新，这对于原型设计十分有用。当你修改一些变量，即使是一个简单的 SVG 多边形也可实现很多难以想象的效果。

可以通过[这个示例](https://codesandbox.io/s/github/vuejs/vuejs.org/tree/master/src/v2/examples/vue-20-dynamic-state-transitions)进行详阅。
...
Sarah Drasner 展示了下面这个 demo，这个 [demo ](https://codepen.io/sdras/pen/YZBGNp)结合了时间和交互相关的状态改变。
